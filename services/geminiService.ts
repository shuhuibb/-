...
// 使用系统要求的 process.env.API_KEY，由 vite.config.ts 处理注入
...
// --- 词汇服务 (保持简洁) ---
export const generateVocabBatch = async (mode: string): Promise<any[]> => {
  const schema: Schema = {
    type: Type.OBJECT,
    properties: {
      items: {
  ...
  try {
    const response = await ai.models.generateContent({
      model: MODEL_TEXT,
      contents: `生成3个韩语练习题。模式: ${mode}。JSON格式。`,
